...
begin
  writeln('': indent, root, '(', c[root], ') - ', w[root]);
  ...
  begin
    ...
    begin
      ...
      begin
        res^[2, 0, i] := maxcost;
        res^[2, 1, i] := c[root];
        if not above[root, i] and (c[i] < maxcost) then
        begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if above[root, curr] then
          begin
            if (q <= lsize) and (c[curr] < maxcost) and (dist[root, curr] + leftVal^[2, q, curr] < res^[2, q, curr]) then
              res^[2, q, curr] := dist[root, curr] + leftVal^[2, q, curr];
            res^[1, q, curr] := maxcost;
          end else
          begin
            res^[1, q, curr] := res^[2, q, curr];
            if (q <= lsize) and (c[curr] < maxcost) and (dist[root, curr] + leftVal^[1, q, curr] < res^[1, q, curr]) then
